...
public class Task {
        ...
        for(int r = 0; r < 7; r++){
            ...
            for(int c = 0; c < 7; c++){
                if (c > r){
                    ...
                    if(newPheromone[r][TARGET_NUMBER] > 0){
                        foundAlternative(road, r, sumOfCost);
                    }
                }
            }
    ...
    private void foundAlternative(List<Character> road, int r, int currentCost) {
        System.out.println("Alternative:");
        System.out.println(currentCost);
        currentCost += cost[r][TARGET_NUMBER];
        List<Character> alternativeRoads = new ArrayList<>(road);
        alternativeRoads.add('G');
        for (Character character : alternativeRoads) {
            String output = character != symbolsName[TARGET_NUMBER] ? character + " -> " : "" + character + "\n";
            System.out.print(output);
        }
    }
